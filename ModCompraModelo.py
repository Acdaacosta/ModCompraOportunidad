...
class ModCompraModelo:
    ...
    #nuevo
            ...
            #values = [0, 25000, 50000, 100000]  # Los cinco valores posibles
        # num_values = len(values)
        # x = [[m.Var(integer=True) for _ in range(num_values)] for _ in range(n)]
            
        # comp_imp_pn = [m.Var(lb=0, ub=p_Cap_compra_imp, integer=True) for _ in range(n)]
            ...
            for i in range(n):
              m.Equation(comp_imp_pn[i]>=0)
            #m.Equation(sum(x[i]) == 0)  # Exactamente una de las x[i] debe ser 1

            #for j in range(num_values):
            #  m.Equation(comp_imp_pn[i] >= values[j] * x[i][j])
            #  m.Equation(comp_imp_pn[i] <= values[j] * x[i][j] + (1 - x[i][j]) * p_Cap_compra_imp)
            ...
            for i in range(n):
                ...
                #nueva restriccion
            # m.Equation(comp_imp_pn[i] == ((comp_imp_pn[i]  ))* 25000)
